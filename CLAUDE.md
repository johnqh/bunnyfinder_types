...
Shared TypeScript type definitions for the BunnyFinder API ecosystem.

**npm**: `@sudobility/entitybunnyfinder_types` (public)
...
```
@sudobility/entitybunnyfinder_types (this package)
    ^
entitybunnyfinder_api
entitybunnyfinder_client
entitybunnyfinder_lib
entitybunnyfinder_app
entityentitybunnyfinder_app_rn
```

Foundation layer — all other BunnyFinder projects depend on this package.

## Related Projects

- **entitybunnyfinder_api** — Backend API server (Hono + PostgreSQL); imports types for request/response validation
- **entitybunnyfinder_client** — API client SDK with TanStack Query hooks; imports types for API contracts
- **entitybunnyfinder_lib** — Business logic library with Zustand stores; imports types transitively via entitybunnyfinder_client
- **entitybunnyfinder_app** — Web application (React + Vite); imports types transitively via entitybunnyfinder_client and entitybunnyfinder_lib
- **entityentitybunnyfinder_app_rn** — React Native mobile app (Expo); imports types via file: links

All other bunnyfinder_* projects depend on this package. This package depends on `@sudobility/types` for base infrastructure types (`BaseResponse`, `NetworkClient`, etc.).
...
- Changes here affect ALL consumer projects (entitybunnyfinder_api, entitybunnyfinder_client, entitybunnyfinder_lib, entitybunnyfinder_app, entityentitybunnyfinder_app_rn) -- always consider downstream impact
